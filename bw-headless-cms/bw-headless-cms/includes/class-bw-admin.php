...
class BW_Admin_Settings {
    ...
    public function enqueue_admin_assets($hook) {
        ...
        wp_add_inline_style('wp-components', '
            ...
            .bw-faq-editor-item { border: 1px solid #ddd; padding: 15px; margin-bottom: 15px; background: #f9f9f9; position: relative; border-radius: 4px; }
            .bw-faq-editor-item .remove-faq { position: absolute; top: 10px; right: 10px; color: #d63638; cursor: pointer; text-decoration: none; font-weight: bold; }
            .bw-faq-editor-item label { display: block; font-weight: 600; margin-bottom: 5px; }
            .bw-faq-editor-item input, .bw-faq-editor-item textarea { width: 100%; margin-bottom: 10px; border: 1px solid #ccc; padding: 8px; border-radius: 2px; }
        ');
    }
        ...
        <div class="wrap bw-wpds-wrap">
            ...
            <?php elseif ($active_tab === 'theme'): ?>
                <div class="components-panel">
                    <div class="components-panel__body">
                        ...
                        <form id="bw-theme-form">
                            ...
                            <h3 style="margin-top: 20px;">Pengaturan FAQ Homepage</h3>
                            <div id="bw-faq-editor-list" style="margin-bottom: 15px;">
                                <!-- FAQ items will be added here via JS -->
                            </div>
                            <button type="button" id="bw-add-faq-item" class="button">+ Tambah Item FAQ</button>

                            <h3 style="margin-top: 20px; border-top: 1px solid #eee; padding-top: 20px;">Pengaturan CTA Footer</h3>
                            <div class="components-base-control components-text-control">
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const faqList = document.getElementById('bw-faq-editor-list');
                    const btnAddFaq = document.getElementById('bw-add-faq-item');
                    
                    function addFaqItem(q = '', a = '') {
                        const div = document.createElement('div');
                        div.className = 'bw-faq-editor-item';
                        div.innerHTML = `
                            <a href="#" class="remove-faq">× Hapus</a>
                            <label>Pertanyaan</label>
                            <input type="text" class="faq-q" value="${q}" placeholder="Tulis pertanyaan..." />
                            <label>Jawaban</label>
                            <textarea class="faq-a" rows="3" placeholder="Tulis jawaban...">${a}</textarea>
                        `;
                        div.querySelector('.remove-faq').onclick = function(e) {
                            e.preventDefault();
                            div.remove();
                        };
                    ...
                    btnAddFaq.onclick = function() { addFaqItem(); };
                    ...
                    Promise.all([
                        ...
                        if (resFaq.data && Array.isArray(resFaq.data)) {
                            resFaq.data.forEach(item => addFaqItem(item.q, item.a));
                        } else if (resFaq.data && typeof resFaq.data === 'string') {
                            try {
                                const parsed = JSON.parse(resFaq.data);
                        ...
                        const selectedPromos = Array.isArray(resPromoBulanan.data) ? resPromoBulanan.data : [];
                        
                        if (Array.isArray(resPromosiActive) && resPromosiActive.length > 0) {
                            resPromosiActive.forEach(promo => {
                                const isChecked = selectedPromos.includes(promo.slug) ? 'checked' : '';
                                const div = document.createElement('div');
                                div.style.marginBottom = '8px';
                                div.innerHTML = `
                                    <label style="display: flex; align-items: center; gap: 8px; cursor: pointer;">
                                        <input type="checkbox" name="promo_bulanan[]" value="${promo.slug}" class="bw-promo-checkbox" ${isChecked} />
                            ...
                            promoListContainer.innerHTML = '<p style="margin: 0; color: #d63638;">Belum ada promosi aktif. Silakan buat promosi terlebih dahulu.</p>';
                        }
                    });
                    ...
                    form.addEventListener('submit', function(e) {
                        ...
                        const faqs = [];
                        document.querySelectorAll('.bw-faq-editor-item').forEach(item => {
                            faqs.push({
                        ...
                        document.querySelectorAll('.bw-promo-checkbox:checked').forEach(cb => {
                        ...
                        Promise.all([
                            ...
                            Promise.all([
                                ...
                                fetch(apiEndpointFaq, {
                                    method: 'PATCH',
                                    headers: { 'Content-Type': 'application/json', 'X-WP-Nonce': '<?php echo wp_create_nonce('wp_rest'); ?>' },
                                    body: JSON.stringify(faqs)
                                }),
                                fetch(apiEndpointPromoBulanan, {
                                    method: 'PATCH',
                                    headers: { 'Content-Type': 'application/json', 'X-WP-Nonce': '<?php echo wp_create_nonce('wp_rest'); ?>' },
