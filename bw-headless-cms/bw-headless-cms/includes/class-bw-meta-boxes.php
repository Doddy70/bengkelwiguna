...
class BW_Meta_Boxes {
    ...
    // =============================================
    // SERVICES FAQ META BOX (NEW)
    // =============================================
    public function add_services_faq_meta_box() {
        add_meta_box(
            'bw_services_faq_meta_box',
            'FAQ (Pertanyaan Umum)',
            [$this, 'render_services_faq_meta_box'],
            'services',
            'normal',
            'high'
        );
    ...
    public function render_services_faq_meta_box($post) {
        wp_nonce_field('bw_save_services_faq_meta', 'bw_services_faq_meta_nonce');

        $faqs_json = get_post_meta($post->ID, 'bw_services_faq', true);
        $faqs = !empty($faqs_json) ? json_decode($faqs_json, true) : [];
        if (!is_array($faqs)) $faqs = [];

        ?>
        <style>
            .bw-services-faq-wrapper { margin-bottom: 20px; }
            .bw-services-faq-item { border: 1px solid #ddd; padding: 15px; margin-bottom: 15px; background: #fafafa; position: relative; border-radius: 5px; }
            .bw-services-faq-item label { display: block; font-weight: bold; margin-bottom: 8px; color: #1d2327; }
            .bw-services-faq-item input[type="text"] { width: 100%; margin-bottom: 10px; padding: 8px; }
            .bw-services-faq-item textarea { width: 100%; margin-bottom: 10px; padding: 8px; min-height: 80px; }
            .bw-services-faq-remove { position: absolute; top: 10px; right: 10px; color: #d63638; text-decoration: none; font-weight: bold; font-size: 20px; }
            .bw-services-faq-remove:hover { color: #b32d2e; }
            .bw-services-faq-header { display: flex; justify-content: space-between; align-items: center; margin-bottom: 15px; padding-bottom: 10px; border-bottom: 2px solid #224297; }
            .bw-services-faq-header h4 { margin: 0; color: #224297; }
            .bw-services-faq-header p { margin: 5px 0 0 0; font-size: 12px; color: #666; }
        </style>

        <div class="bw-services-faq-wrapper">
            <div class="bw-services-faq-header">
                <div>
                    <h4>FAQ Layanan</h4>
                    <p>Tambahkan pertanyaan dan jawaban yang sering ditanyakan pelanggan tentang layanan ini.</p>
                </div>
            </div>
            <div id="bw-services-faq-list">
                <?php if (empty($faqs)): ?>
                <div class="bw-services-faq-item">
                    <a href="#" class="bw-services-faq-remove" title="Hapus">&times;</a>
                    <label>Pertanyaan</label>
                    <input type="text" name="bw_services_faqs[0][q]" value="" placeholder="Contoh: Berapa lama waktu pengerjaan?" />
                    <label>Jawaban</label>
                    <textarea name="bw_services_faqs[0][a]" rows="3" placeholder="Contoh: Waktu pengerjaan biasanya 1-2 jam tergantung kondisi kendaraan."></textarea>
                </div>
                <?php else: ?>
                <?php foreach ($faqs as $index => $faq): ?>
                <div class="bw-services-faq-item">
                    <a href="#" class="bw-services-faq-remove" title="Hapus">&times;</a>
                    <label>Pertanyaan</label>
                    <input type="text" name="bw_services_faqs[<?php echo $index; ?>][q]" value="<?php echo esc_attr($faq['q']); ?>" placeholder="Masukkan pertanyaan..." />
                    <label>Jawaban</label>
                    <textarea name="bw_services_faqs[<?php echo $index; ?>][a]" rows="3" placeholder="Masukkan jawaban..."><?php echo esc_textarea($faq['a']); ?></textarea>
                </div>
                <?php endforeach; ?>
                <?php endif; ?>
            </div>
            <button type="button" class="button button-primary" id="bw_services_add_faq_btn" style="background: #224297; border-color: #224297;">+ Tambah FAQ</button>
        </div>
        ...
        jQuery(document).ready(function($) {
            ...
            $('#bw_services_add_faq_btn').on('click', function(e) {
                e.preventDefault();
                var html = '<div class="bw-services-faq-item">' +
                    '<a href="#" class="bw-services-faq-remove" title="Hapus">&times;</a>' +
                    '<label>Pertanyaan</label>' +
                    '<input type="text" name="bw_services_faqs[' + faqIndex + '][q]" value="" placeholder="Masukkan pertanyaan..." />' +
                    '<label>Jawaban</label>' +
                    '<textarea name="bw_services_faqs[' + faqIndex + '][a]" rows="3" placeholder="Masukkan jawaban..."></textarea>' +
                    '</div>';
                $('#bw-services-faq-list').append(html);
                faqIndex++;
            });

            $('#bw-services-faq-list').on('click', '.bw-services-faq-remove', function(e) {
                e.preventDefault();
                $(this).closest('.bw-services-faq-item').remove();
            });
        });
        </script>
        <?php
    ...
    public function save_services_faq_meta_box($post_id) {
        if (!isset($_POST['bw_services_faq_meta_nonce']) || !wp_verify_nonce($_POST['bw_services_faq_meta_nonce'], 'bw_save_services_faq_meta')) {
            return;
        }
        if (defined('DOING_AUTOSAVE') && DOING_AUTOSAVE) {
            return;
        ...
        if (isset($_POST['bw_services_faqs']) && is_array($_POST['bw_services_faqs'])) {
            $sanitized_faqs = [];
            foreach ($_POST['bw_services_faqs'] as $faq) {
            ...
            if (!empty($sanitized_faqs)) {
                update_post_meta($post_id, 'bw_services_faq', json_encode($sanitized_faqs));
            } else {
                delete_post_meta($post_id, 'bw_services_faq');
            }
        } else {
            delete_post_meta($post_id, 'bw_services_faq');
        }

        // Clear transient cache for services
