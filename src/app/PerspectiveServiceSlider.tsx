...
const PerspectiveServiceSlider: React.FC<PerspectiveServiceSliderProps> = ({ servicesData = [] }) => {
  ...
  const getCardStyle = (index: number) => {
    ...
    // Adjusted spread spacing
    const spreadSpacing = isMobile ? 220 : isTablet ? 280 : 340;
      ...
      <div className="h5-banner-area relative z-10">
        <div className="container mx-auto">
          ...
          <div className="sec-heading-wrap mb-6 sm:mb-16 text-center flex flex-col items-center justify-center px-4">
            {/* Glass Badge with Glow */}
            <div className="relative mb-4 sm:mb-8 group">
              {/* Outer Glow */}
              <div className="absolute inset-0 bg-gradient-to-r from-[#224297]/30 via-[#ffd900]/20 to-[#224297]/30 blur-2xl rounded-full scale-125 opacity-60" />

              {/* Glass Badge */}
              <div className="relative bg-white/70 backdrop-blur-xl px-6 sm:px-10 py-3 sm:py-4 rounded-full border border-white/60 shadow-[0_8px_32px_rgba(34,66,151,0.15)] flex items-center gap-2 sm:gap-3">
                {/* Glass Shimmer */}
                <div className="absolute inset-0 rounded-full overflow-hidden">
                  <div className="glass-shimmer" />
                </div>

                <span className="text-[#ffd900] text-sm sm:text-lg drop-shadow-md animate-pulse relative z-10">✨</span>
                <span className="text-[#224297] text-[11px] sm:text-sm md:text-base font-bold tracking-wide sm:tracking-widest uppercase leading-snug relative z-10">
            ...
            <h1 className="sec-title mb-4 sm:mb-6 text-3xl sm:text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-black italic tracking-tight leading-[1.1]">
              <span className="block">
                di <span className="bg-gradient-to-r from-[#224297] to-[#3d5afe] bg-clip-text text-transparent">Bengkel Wiguna</span>
              </span>
              {/* Decorative Glass Underline */}
              <span className="block relative mt-2">
                <span className="absolute left-1/2 -translate-x-1/2 w-32 sm:w-48 h-1.5 bg-gradient-to-r from-transparent via-[#ffd900] to-transparent rounded-full" />
              </span>
            </h1>

            {/* Glass Subtitle */}
            <p className="text-gray-600/80 text-xs sm:text-sm md:text-base font-medium tracking-wide max-w-[320px] sm:max-w-none mx-auto leading-relaxed">
              One Stop Service • Ban • Oli • AC • Kaki-Kaki • Spooring • Balancing
            </p>
          </div>
                  ...
                  return (
                    ...
                    >
                      <Link
                        ...
                        className="relative w-full h-full rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3rem] overflow-hidden block group"
                        onClick={(e) => {
                            if (!isCenter) {
                                e.preventDefault();
                            }
                      ...
                      >
                        {/* LIQUID GLASS CARD */}
                        <div className="absolute inset-0 rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3rem] overflow-hidden">

                          {/* Glass Background */}
                          <div className="absolute inset-0 bg-gradient-to-br from-white/25 via-white/15 to-white/20 backdrop-blur-[20px]" />
                          ...
                          <Image
                              ...
                              className="transition-transform duration-1000 ease-[cubic-bezier(0.32,0.72,0,1)] opacity-90 group-hover:opacity-100 group-hover:scale-105"
                          />

                          {/* Gradient Overlay for Text */}
                          <div className="absolute inset-0 bg-gradient-to-t from-black/80 via-black/40 to-black/20 rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3rem]" />

                          {/* Glass Highlight at Top */}
                          <div className="absolute inset-x-0 top-0 h-1/2 bg-gradient-to-b from-white/15 to-transparent rounded-t-[2rem] sm:rounded-t-[2.5rem] md:rounded-t-[3rem] pointer-events-none" />

                          {/* Glass Border Effect */}
                          <div className="absolute inset-0 rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3rem] border border-white/20 pointer-events-none" />

                          {/* Shimmer Effect */}
                          <div className="absolute inset-0 overflow-hidden rounded-[2rem] sm:rounded-[2.5rem] md:rounded-[3rem] pointer-events-none">
                            <div
                        ...
                        <div className="absolute inset-x-0 bottom-0 p-4 sm:p-5 md:p-8 flex flex-col justify-end z-10">
                            <p className="text-[#ffd900] font-black text-xs sm:text-sm md:text-xl mb-0.5 md:mb-1 tracking-widest drop-shadow-lg">
                                #{String(index + 1).padStart(2, '0')}
                            </p>
                            <h4 className="text-white font-bold text-sm sm:text-lg md:text-2xl leading-tight drop-shadow-lg mb-1 sm:mb-2 md:mb-4">
                                {title}
                            </h4>

                            {/* LIQUID GLASS BUTTON */}
                            <div className={`transition-all duration-500 overflow-hidden ${isCenter ? 'max-h-14 sm:max-h-16 opacity-100' : 'max-h-0 opacity-0'}`}>
                                <div className="relative w-full py-2 sm:py-2.5 md:py-3.5 rounded-full overflow-hidden">
                                    {/* Glass Button Background */}
                                    <div className="absolute inset-0 bg-gradient-to-r from-[#224297]/90 to-[#224297]/70 backdrop-blur-md" />
                                    ...
                                    {/* Button Content */}
                                    <div className="relative z-10 flex items-center justify-center w-full text-white text-[10px] sm:text-xs md:text-sm font-bold">
                                        Lihat Detail <Icon icon="solar:arrow-right-linear" className="ml-1 sm:ml-1.5 text-sm md:text-lg transition-transform group-hover:translate-x-1" />
                                    </div>
                                </div>
                            </div>
                        </div>
