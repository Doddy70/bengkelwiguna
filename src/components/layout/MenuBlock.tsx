...
/**
 * Desktop Menu Component - Only visible on large screens (md and up)
 */
const DesktopMenu = ({ menuItems }: { menuItems: MenuItem[] }) => {
    return (
        <ul className="hidden md:flex gap-6 font-semibold text-[17px] main-menu">
...
/**
 * Mobile Menu Component - Only visible on small/medium screens
 */
const MobileMenu = ({
    menuItems,
    mobileOpen,
    ...
    return (
        <div className="fixed top-0 left-0 h-full w-72 bg-white z-50 transform transition-transform duration-300 flex flex-col">
            {/* Header with Logo and Close Button */}
            <div className="flex items-center justify-between p-4 border-b border-gray-200">
                <Link href="/" className="flex items-center">
                    <Image
                        src={logo || "/images/logo/logo-square.avif"}
                        alt="logo"
                        width={120}
                        height={40}
                        priority
            ...
            {/* Menu Items */}
            <ul className="flex flex-col p-4 gap-2 overflow-y-auto flex-1">
                {menuItems.map((item, index) => (
                    <li key={index}>
                                ...
                                {openSubMenu[`${index}`] && (
                                    <ul className="pl-4 mt-2 flex flex-col gap-2">
                                        {item.subMenu &&
                                            item.subMenu.map((sub, subIndex) => (
                                                <li key={subIndex}>
                                                    <Link href={sub.href || '#'} className="text-gray-700 py-1 block font-medium hover:text-brand-blue" onClick={toggleMobileMenu}>
                                                        {sub.title}
                                                    </Link>
                                                </li>
                                            ))}
                                        ...
                                        {item.megaMenu &&
                                            item.megaMenu.map((section, secIndex) => (
                                                <li key={secIndex}>
                                                    <button
                                                        onClick={() => toggleSubMenu(`${index}-${secIndex}`)}
...
/**
 * MenuBlock - Main component that combines Desktop and Mobile menus
 */
const MenuBlock: React.FC<MenuBlockProps> = ({
    ...
    return (
        <>
            {/* Desktop Menu - Only visible on large screens (lg and up) */}
            <DesktopMenu menuItems={menuItems} />

            {/* Mobile Menu - Only visible when mobileOpen is true (shown via Header overlay) */}
            <MobileMenu
                menuItems={menuItems}
