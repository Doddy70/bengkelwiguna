...
const rightWidgetRegex = /\{\/\* Top Right: Specs Widget \*\/\}([\s\S]*?)<\/motion\.div>\n/m;
...
const newRightWidget = `{/* Right Side Medical Report Card */}
            ...
            >
              ...
              {/* Thumbnail Gallery (X-Ray Style with Real Images) */}
              <div className="flex gap-2">
                 {["/images/hotspot/Fortuner_Kyoto.png", "/images/hotspot/fortuner_ac.png", "/images/hotspot/fortuner_coolant.png", "/images/hotspot/fortuner_stinger.png"].map((imgSrc, i) => (
                    <div key={i} className="flex-1 aspect-square bg-white rounded-lg overflow-hidden relative group cursor-pointer border border-gray-200">
                       <Image 
                         src={imgSrc} 
                         alt="Thumbnail" 
                         fill 
                       ...
                       />
                       <div className="absolute inset-0 bg-[#224297] opacity-0 group-hover:opacity-10 transition-opacity"></div>
                    </div>
                 ))}
              </div>
              ...
              <div className="grid grid-cols-2 gap-3">
                 ...
                 <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                   <div className="flex items-center justify-between mb-1">
                     <h4 className="text-[10px] font-bold text-gray-500">{activeHotspot ? activeHotspot.widgetMetric1?.label : activeItem.specs?.metric1?.label || "Akurasi"}</h4>
                   ...
                   <div className="flex items-end gap-1">
                     <p className="text-[18px] font-bold text-[#224297] leading-none whitespace-nowrap overflow-visible">
                        {activeHotspot ? activeHotspot.widgetMetric1?.value : activeItem.specs?.metric1?.value || "99"}
                     </p>
                   </div>
                 ...
                 <div className="bg-gray-50 rounded-xl p-3 border border-gray-100">
                   <div className="flex items-center justify-between mb-1">
                     <h4 className="text-[10px] font-bold text-gray-500">{activeHotspot ? activeHotspot.widgetMetric2?.label : activeItem.specs?.metric2?.label || "Garansi"}</h4>
                     <FeatherIcons.Thermometer size={10} className="text-gray-400" />
                   </div>
                   <div className="flex items-end gap-1">
                     <p className="text-[18px] font-bold text-[#224297] leading-none whitespace-nowrap overflow-visible">
                        {activeHotspot ? activeHotspot.widgetMetric2?.value : activeItem.specs?.metric2?.value || "Basic"}
                     </p>
                   </div>
                   <div className="h-1 w-full bg-gray-200 rounded-full mt-2 overflow-hidden">
...
console.log('Fixed Right Widget Replacement with real images');
