...
Before proceeding, validate all four inputs. Return a clear error and do NOT
...
Input schema:
  plugin_checkout_path:
    ...
    validation:
      ...
      - Path must contain at least one .php file (confirm plugin, not empty dir)

  triage_output:
    type: object | null
    required: true
    note: "May be null if wp-project-triage has never run; audit is still valid"
    validation:
      - If object, must contain fields: signals.usesAbilitiesApi (bool),
        versions.wordpress (string), project.kind (string)
  ...
  output_path:
    ...
    validation:
      ...
      - Must NOT be inside the plugin checkout tree
        (reject: output_path starts with plugin_checkout_path prefix)
...
**Escalation on null triage_output:**
If `triage_output` is `null` (wp-project-triage never run), proceed with
the audit but note in the audit doc's "Notes and Surprises":
> "Triage output not available. `signals.usesAbilitiesApi`, `versions.wordpress`,
> and `project.kind` could not be verified. Audit author should run
> wp-project-triage and backfill these fields."

## Cost controls

The audit procedure has 7 steps. Set hard limits:

```yaml
Cost ceiling:
  max_reference_files_loaded: 7        # One per step; fail if exceeded
  max_controller_inventory_size: 200   # If glob/grep finds >200 controllers,
                                       # stop and report: "plugin has
                                       # unusually large REST surface;
                                       # consider auditing by sub-package"
  max_proposed_abilities: 50          # If proposed_abilities exceeds 50,
                                       # stop and report: "grouping produced
                                       # N abilities; consider whether
                                       # some routes are internal transport"
  max_audit_doc_size_kb: 256           # If compiled doc exceeds 256KB,
                                       # warn: "large audit may strain context
                                       # windows downstream"
  abort_on_ref_file_missing: true     # Any missing reference file = FAIL
                                       # immediately with clear error
```

## Timeout controls

Each step has a maximum wall-clock time. Exceeding the timeout = step FAIL:

```yaml
Timeouts per step:
  step_1_enumerate:    60s   # Glob + grep for controllers
  step_2_extract:       120s  # Read controller files and extract backing fields
  step_3_capability:    60s   # Trace permission_callbacks
  step_4_propose:       180s  # Semantic grouping + fill proposed_abilities schema
                              # (most time-consuming — complex reasoning step)
  step_5_gaps:          30s   # Surface gaps and deferred items
  step_6_write:         60s   # Write and validate audit doc
  step_7_reference:     30s   # Designate reference ability (optional step)
  TOTAL_MAX:            480s  # ~8 minutes wall-clock for full audit

Timeout response:
  Error: "Step N (<step_name>) timed out after <elapsed>s"
  Action: Report partial progress if any data was collected; otherwise
          return "Audit timed out — insufficient data to produce a valid doc."
```

## Error handling patterns

Three patterns apply across all steps:

```yaml
# Pattern A — Reference file missing
Error: "Required reference file not found: <path>"
Action: FAIL immediately with the reference path.
       Report: "Cannot continue — <reference> is missing. Escalate: update
       the reference file in the skill repo before re-running."

# Pattern B — Glob / grep finds nothing (step 1)
Error: "No REST controllers found in <plugin_path>"
Action: Return "No controllers enumerated. Audit does not apply — plugin has
        no REST surface. Failure mode: plugin has no REST controllers."

# Pattern C — Schema field missing (step 4)
Error: "Proposed ability '<name>' is missing required field '<field>'"
Action: Continue audit but mark the ability as PROVISIONAL in the YAML.
       Add to the ability's notes: "PROVISIONAL — <field> not determined at audit time.
       Implementer must fill before registering."
```
...
```yaml
Circuit breaker:
  # Step 6 (write audit doc) is the only stateful step — it writes to output_path.
  # If write fails 2 consecutive times, trip the circuit.
  failure_threshold: 2
  failures:
    - "Cannot write to output_path (permission denied)"
    - "output_path is not writable"
    - "disk full during write"
  state: CLOSED → OPEN (after 2 consecutive write failures)
  Action on OPEN:
    Error: "Circuit OPEN: output_path write failed 2 times. Abort audit."
    Action: Report to user: "Audit cannot complete — output path is
            inaccessible. Please verify the output path is writable and
            retry with a corrected path."
    Cooldown: User must re-invoke the skill with a new output_path.
```
...
**Timeout: 60s | Circuit breaker: none at this step**
...
**On zero controllers found:**
Return "No REST controllers found. Audit does not apply." — do NOT proceed
to step 2.

**On >200 controllers found:**
Return "Plugin has [N] REST controllers (threshold: 200). Consider auditing
by sub-package or namespace rather than the full repo." — do NOT proceed
until the user narrows the scope.

### 2. For each controller, extract the backing fields

**Timeout: 120s | Circuit breaker: none**
...
**On missing reference file:**
FAIL immediately. Do not attempt to continue.

### 3. Confirm capability gate(s)

**Timeout: 60s | Circuit breaker: none**
...
**Timeout: 180s | Circuit breaker: none**
...
**On proposed_abilities > 50:**
Return "Grouping produced [N] proposed abilities (threshold: 50). Some
routes may be internal transport plumbing. Review: each proposed ability
must pass the use-case-contract test. Reduce to ≤50 before writing."

**On missing required field in a proposed ability:**
Continue but mark ability as PROVISIONAL. Add to notes:
`PROVISIONAL — <field> not determined at audit time.`

### 5. Surface gaps and deferred items

**Timeout: 30s | Circuit breaker: none**
...
**Timeout: 60s | Circuit breaker: WRITE (trip after 2 failures)**
...
**On doc exceeds 256KB:**
Add note at top of audit doc: `NOTE: This audit doc exceeds 256KB.
Downstream tooling may need to paginate or summarize for context-window fit.`

**Circuit breaker trigger — write failure:**
After 2 consecutive write failures, trip the circuit. Report:
"Circuit OPEN: output_path write failed 2 times. Abort audit. Provide
a different output_path and re-invoke."
...
- **Output path defaults to plugin worktree** — always ask the user for an
  ...
  artifact.
- **Step timeout exceeded** — see "Timeout controls" above. Partial progress
  may be reportable; if no data was collected, return a timeout error.
- **Reference file missing mid-procedure** — fail immediately. Do not attempt
  to continue. Report the missing file path.
...
- If proposed_abilities exceeds 50 (cost control threshold) — report to user
  before proceeding. Do not auto-truncate.
- If the audit doc exceeds 256KB — warn in the doc header; do not truncate.