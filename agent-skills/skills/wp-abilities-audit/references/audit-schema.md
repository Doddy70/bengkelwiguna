...
- **Implementation-readiness fields added 2026-05-21.** `use_case_fit`,
  ...
  NOT FAIL, mirroring the legacy `capability_gate` posture above.
  - **Note on `null` vs. populated:** `seed_data_needs` has type
    `string | null` — `null` is technically a valid YAML value and does
    not trigger a WARN on its own (the field is present). However, a
    `null` value means the auditor has not identified the seed shape;
    validators treat it as "ask the implementer" rather than as a
    populated fact. New audits SHOULD populate all three fields with
    real content; `null` is accepted but signals an incomplete audit.
