...
navigate appropriately. The plugin's own in-tree contract tests are
the durable home for these assertions on every CI run — this Level 2
check is the one-off harness run that produces the PR artifact. For
example, WooCommerce extensions use `test-the-public-contract.md` in
their overlay; other plugin families use their own test filenames. The
harness does not assume this file exists — if the plugin has no such
contract test, skip Level 2's contract-test assertion and proceed to
the other checks.
