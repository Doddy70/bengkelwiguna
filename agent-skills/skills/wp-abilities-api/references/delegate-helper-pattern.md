...
- `references/plugin-family-patterns.md` — choosing the helper's constructor shape.
- `references/error-code-vocabulary.md` — the `<plugin>_not_initialized` code and its siblings.
- `references/input-schema-gotchas.md` — callbacks for list abilities often need `per_page` pagination translation BEFORE calling the helper.
